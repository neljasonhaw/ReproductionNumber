...
The rest of this guide uses dummy data. If you want to recreate all of the analyses using the dummy data, Go to the [**Github repository here**](https://github.com/neljasonhaw/ReproductionNumber). Click on the green button `Code` then click `Download ZIP`. Unzip and look for the file name `linelist.csv`. All of the other files are not relevant for the guide or are output files that will be generated as part of the analysis.
...
## Loading packages
We will need to load the relevant R packages first. Make sure that the packages are installed prior to loading them. On the script file you just created, type the following chunk of code:
