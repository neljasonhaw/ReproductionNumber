...
The rest of this guide uses dummy data. If you want to recreate all of the analyses using the dummy data, Go to the [**Github repository here**](https://github.com/neljasonhaw/ReproductionNumber). Click on the green button `Code` then click `Download ZIP`. Unzip and look for the file name `linelist.csv`. 
...
For this guide, we will use a hypothetical example from hypothetical literature where the serial interval was determined to have a Weibull distribution, with mean 7.0 days and standard deviation 5.5 days.
